...
func (tC *testConn) ShiftN(n int) int {
	tC.bytes = tC.bytes[n:]
	return n
}

func (tC *testConn) ResetBuf() {
	return
}
func (tC *testConn) Len() int {
	return 0
}
func (tC *testConn) AsyncWrite(p []byte) error {
	return nil
}
...
func TestLengthFieldBasedFrameCodec64NoPool(t *testing.T) {
	...
	//-----------------lengthField=64----------------------//
	{
		...
		for i := 0; i < loop; i++ {
			...
			//bufferPool.PutBuffer(&bytebufferpool.ByteBuffer{
			//	B: nbyte,
			//})
			slicePool.Put(nbyte)
		}
